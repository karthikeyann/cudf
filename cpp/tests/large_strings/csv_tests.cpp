...
// Disabled because it takes about a minute: the thread of the row parses the 2 GiB field alone.
// Run it with --gtest_also_run_disabled_tests.
TEST_F(CsvLargeReaderTest, DISABLED_FieldLongerThanStringSizeLimit)
{
  // A field of more characters than a string can have is rejected, rather than truncated
  auto const length = static_cast<std::size_t>(std::numeric_limits<cudf::size_type>::max()) + 1;
  std::string text  = "a\n";
  text.append(length, 'x');
  text += "\nb\n";
  EXPECT_THROW(read_string_column(text), std::overflow_error);
}
