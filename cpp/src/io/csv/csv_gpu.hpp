 ...
 * followed by `remove_blank_rows`, but resolves the parser state of each character block in one pass over the data and only
 ...
 * @param stream CUDA stream used for device memory operations and kernel launches
 * @return Offsets of the non-blank rows (see remove_blank_rows) relative to the start of `data`,
 * including the end-of-data offset
 */
